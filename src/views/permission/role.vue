<template>
  <div class="app-container">
    ...
    <el-table :data="rolesList" style="width: 100%;margin-top:30px;" border>
      <el-table-column align="center" label="职位名称" width="220">
    ...
    <el-dialog
      ...
      :title="dialogType === 'edit' ? '编辑角色' : '创建角色'"
    >
      <el-form :model="role" label-width="80px" label-position="left">
        ...
        <el-form-item label="职位">
          <el-input v-model="role.worker" placeholder="职位名称" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async confirmRole() {
      ...
      if (this.role.id === 0) {
        this.role.id = this.rolesList.length + 1
        this.role.key = this.role.name
      }
      ...
      if (isEdit) {
        ...
        await addUser(this.role)
        await await this.getRoles()
      }
      // commit("SET_ROUTES", newRouterList);

      const { worker, workerDes, name } = this.role
